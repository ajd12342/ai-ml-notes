...
- At test time, batch norm uses static mean and variance. This is done in a multitude of ways: Use mean, variance of the entire training set / maintain an exponentially weighted running average of mean and variance while training and use the values at end of training(won't this method localize to the last few iterations, hence bad?) This doesn't cause too much of an issue since we expect train/test sets to come from similar distributions so this estimated mean and variance is probably close enough to what should actually be used at test time.
